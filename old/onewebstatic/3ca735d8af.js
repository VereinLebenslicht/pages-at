...
(function (factory) {
  ...
  (function (window, document, $, undefined) {
    ...
    function Shinybox(element, options) {
      ...
      if ($.isArray(element)) {
        ...
        function openShinyboxOnClick(e) {
          ...
          var initialTargetNode = getAncestorOrSelfWithDataSpecificKind(
            event.target,
          );
          var currentTargetNode = getAncestorOrSelfWithDataSpecificKind(
            event.currentTarget,
          );
          ...
          if (relVal && relVal !== "nofollow") {
            $slideElements = $selectedElements.filter(
              "[" + relType + '="' + relVal + '"]',
            );
          } else {
          ...
          var index =
            typeof $this.data("dom-index") !== "undefined"
              ? $this.data("dom-index") - $(filteredSlides[0]).data("dom-index")
              : $(filteredSlides).index($this);
          var slides = Array.prototype.map.call(
            ...
            function (slideElement) {
              ...
              return {
                ...
                forceMediaType:
                  $slideElement.attr("data-force-media-type") || null,
                forceMediaUrl:
                  $slideElement.attr("data-force-media-url") || null,
                mediaAlbumItems:
                  $slideElement.attr("data-media-album-items") || [],
              };
            },
          );
        ...
        self.destroy = function () {
          $selectedElements &&
            $selectedElements.off("click", openShinyboxOnClick);
          self.ui && self.ui.destroy();
        };
        self.refresh = function () {
          self.destroy();
    ...
    UI.prototype.build = function () {
      this.overlay = $(
        '<div id="' + this.settings.id + '"  class="shinybox-overlay" />',
      );
      ...
      var touchMoveEventHandler = function (e) {
        ...
        if (!hSwipe && !self.settings.verticalSwipeDisable) {
          ...
          if (Math.abs(vDistance) >= vSwipMinDistance || vSwipe) {
            ...
            self.slider.css({ top: vDistance + "px", opacity: opacity });
            vSwipe = true;
          }
        }
        ...
        if (hSwipe) {
          if (0 < hDistance) {
            self.moveSlider(
              self.currentX + (0 === index ? 4 : hDistancePercent),
            );
          } else if (0 > hDistance) {
            self.moveSlider(
              self.currentX +
                (self.slides.length === index + 1 ? -4 : hDistancePercent),
            );
          }
        }
      };
      ...
      this.overlay.on("touchend", function (e) {
        ...
        if (vSwipe && !self.settings.verticalSwipeDisable) {
          ...
          if (
            ...
            var vOffset =
              vDistance > 0 ? self.slider.height() : -self.slider.height();
            self.slider.animate(
            ...
            self.slider.animate({ top: 0, opacity: 1 }, 300);
          }
        } else if (hSwipe) {
          hSwipe = false;
          if (
    ...
    UI.prototype.setSlide = function (index, isFirst) {
      ...
      this.settings.afterSlide(
        this.slideElements.filter(".current").children(),
        index,
      );
    };
    ...
    UI.prototype.getNext = function () {
      ...
      var nextIndex =
        currentIndex + 1 < this.slides.length ? currentIndex + 1 : 0;
    ...
    UI.prototype.getPrev = function () {
      ...
      var currentIndex = this.getCurrentIndex();
      var prevIndex =
        currentIndex > 0 ? currentIndex - 1 : this.slides.length - 1;
    ...
    UI.prototype.resetIframeInSlide = function (index) {
      index = this.validateIndex(index);
      var iframeInSlide = this.slideElements
        .eq(index)
        .contents()
        .find("iframe");
    ...
    UI.prototype.loadMedia = function (src, srcset, callback) {
      ...
      if (src.trim().indexOf("#") === 0) {
        callback(
          $('<div class="shinybox-inline-container" />').append($(src).clone()),
        );
      } else {
    ...
    UI.prototype.loadAlbums = function (albumItems, callback) {
      ...
      function createVideoElement(item) {
        var videoTag = $("<video />", {
          ...
          $(videoTag).height(
            $(videoTag).parents(".album-container").height() - 50,
          );
          albumLeftNav.css("display", "inline-block");
          albumRightNav.css("display", "inline-block");
        });
    ...
    UI.prototype.getPDF = function (url) {
      ...
      return (
        '<div class="shinybox-pdf-container"><div class="shinybox-pdf">' +
        iframe +
        "</div></div>"
      );
    };
    ...
    UI.prototype.getVideo = function (url) {
      ...
      if (youtubeUrl || youtubeShortUrl) {
        ...
        iframe =
          '<iframe width="560" height="315"  src="/old/player.vimeo.com/video/' +
          vimeoUrl[1] +
          "?" +
          qs +
      ...
      return (
        '<div class="shinybox-video-container" style="max-width:' +
        this.settings.videomaxWidth +
        'px"><div class="shinybox-video">' +
        iframe +
        "</div></div>"
      );
    };
